...
function getDockerLoginToECRCommand() {
  const result = awsCommand(['ecr', 'get-login-password', '--region', this.options.region]);
  // AWS CLI returns the full command with "docker " out front. Remove it since we don't need it.
  const password = result.stdout.replace('docker ', '').replace(/[\n\r]/g, '');
  console.log('dockerLoginResult: password', password);
  const acctId = getAWSAccountID();
  const loginString = `login -u AWS -p ${password} https://${acctId}.dkr.ecr.us-east-1.amazonaws.com`'
  console.log('dockerLoginResult: loginString', loginString);
  return loginString;
}
