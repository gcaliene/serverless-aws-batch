...
function buildDockerImage() {
  ...
  _.forEach(
    ...
    fn => {
      ...
      fse.copyFileSync(path.join(packagePath, fileName), path.join(tmpPath, fileName));
  ...
  const dockerFile = path.join(tmpPath, "Dockerfile");
  ...
  const dockerOptions = [
    ...
    dockerFile,
    "-t",
    this.provider.naming.getDockerImageName(),
    tmpPath,
    "--no-cache",
  ];
...
function pushDockerImageToECR() {
  ...
  dockerCommand(
    ["push", this.provider.naming.getECRRepositoryURL()],
    null,
    "inherit"
  );
